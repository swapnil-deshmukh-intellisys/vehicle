  ...
  if (typeof Hammer !== 'undefined') {
    ...
    $(document).on('shown.bs.modal', '.modal', function () {
      $(this).find('.select2').each(function () {
        // WIP select fix
        $('.select2-container--open').css('z-index', $('.modal:visible').css('z-index'));
      });
    });

    // Fix scroll after modal close
    $(document).on('hidden.bs.modal', '.modal', function () {
      // Only remove if no Select2 dropdowns are currently open
      if (!$('.select2-dropdown').is(':visible')) {
        $('body').removeAttr('data-select2-id');
      }
    });

    // Fix scroll after Select2 dropdown close
