...
$(window).on('load', function () {
  ...
  var chartWrapper = $('.chartjs'),
    ...
    analyticsLeadByStagesChart = $('.analytics-lead-by-stages-chart'),
  ...
  // Analytics Lead By Stages Chart
  // --------------------------------------------------------------------
  if (analyticsLeadByStagesChart.length) {
    var leadByStagesSelect = $('#leadByStagesSelect');
    var leadStatusPerYear = JSON.parse($('#lead-status-per-year').text());
    ...
    function populateMonthYears() {
        var monthYears = new Set();
        var currentMonthYear = getCurrentMonthYear();
        var currentMonthYearExists = false;
        
        Object.keys(leadStatusPerYear).forEach(year => {
            Object.keys(leadStatusPerYear[year]).forEach(month => {
                var monthYear = month + '-' + year;
                monthYears.add(monthYear);
                if (monthYear === currentMonthYear) {
                    currentMonthYearExists = true;
                }
            });
        });
        
        leadByStagesSelect.empty();
        monthYears.forEach(monthYear => {
            leadByStagesSelect.append(new Option(monthYear, monthYear, false, monthYear === currentMonthYear));
        });
        
        if (!currentMonthYearExists) {
            leadByStagesSelect.append(new Option(currentMonthYear, currentMonthYear, true, true));
        }
    }
    ...
    function updateChartData(selectedData) {
        ...
        var labels = Object.keys(selectedData).map(label => label.charAt(0).toUpperCase() + label.slice(1));
        var values = Object.values(selectedData);

        chartInstance = new Chart(analyticsLeadByStagesChart, {
            type: 'horizontalBar',
            options: {
                elements: {
                    rectangle: {
    ...
    function loadChartData(monthYear = '') {
      var selectedData = null;
      var noDataMessage = $('#leadByStagesNoData');
      var [selectedMonth, selectedYear] = monthYear.split('-');
  
      if (leadStatusPerYear[selectedYear] && leadStatusPerYear[selectedYear][selectedMonth]) {
          selectedData = leadStatusPerYear[selectedYear][selectedMonth];
      }
      ...
      if (selectedData) {
          ...
          noDataMessage.show(); // Show only if there's no data
      }
    }  

    populateMonthYears();
    loadChartData(getCurrentMonthYear());

    leadByStagesSelect.on('change', function () {
