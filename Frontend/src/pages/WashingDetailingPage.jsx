...
const FALLBACK_WASHING_SERVICE_ID = 8;

const WashingDetailingPage = () => {
  ...
  useEffect(() => {
    ...
    if (vehicleType === 'two-wheeler' || vehicleType === 'four-wheeler') {
      ...
      navigate(`/washing-detailing?${newSearchParams.toString()}`, { replace: true });
    }
  ...
  useEffect(() => {
    if (!locationReady || !washingServiceCategoryId) return;
      ...
      try {
        ...
        const requestData = {
          ...
          filter: {
            ...
            ratings: [],
            distence: [],
            service: [washingServiceCategoryId],
          },
        };
    ...
    fetchWashingGarages();
  }, [locationReady, selectedCity, washingServiceCategoryId]);
  ...
  const handleGarageClick = (garage) => {
    navigate(`/washing-detailing/${garage.id}`);
  };
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden w-full max-w-full ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
      {/* Washing & Detailing Centers Section */}
        ...
        <div className="max-w-7xl mx-auto relative z-10 w-full">
          <div className="text-center mb-8">
            <h2 className={`text-2xl md:text-3xl font-bold mb-2 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
              Washing & Detailing Centers
            </h2>
            <p className={`text-sm md:text-base ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
              Find professional car wash and detailing services
            </p>
          </div>
              ...
              {filteredGarages.length === 0 ? (
                <div className="text-center py-12">
                  <p className={`text-lg ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                    No washing & detailing centers found matching your criteria.
                  </p>
                </div>
              ) : (
...
export default WashingDetailingPage;
