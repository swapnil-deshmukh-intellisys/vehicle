              ...
              <div className={`border-t pt-3 ${theme === 'light' ? 'border-gray-200' : 'border-gray-700'}`}>
                <div className="flex space-x-2">
                  <button
                    onClick={() => {
                      if (isAuthenticated()) {
                        navigate(`/booking?garageId=${id || garageFromState?.id}&serviceType=ev-service`);
                      } else {
                        navigate('/login', { state: { returnTo: `/ev-service/${id || garageFromState?.id}` } });
                      }
                    }}
...
export default EVServiceInfoPage;
