...
const FALLBACK_EMERGENCY_SERVICE_ID = 5;
...
const RoadsideAssistancePage = () => {
  ...
  useEffect(() => {
    ...
    if (vehicleType === 'two-wheeler' || vehicleType === 'four-wheeler') {
      ...
      navigate(`/roadside-assistance?${newSearchParams.toString()}`, { replace: true });
    }
  ...
  useEffect(() => {
    if (!locationReady || !emergencyServiceCategoryId) return;
      ...
      try {
        ...
        const requestData = {
          ...
          filter: {
            ...
            ratings: [],
            distence: [],
            service: [emergencyServiceCategoryId],
          },
        };
  ...
  }, [locationReady, selectedCity, emergencyServiceCategoryId]);
  ...
  const handleGarageClick = (garage) => {
    navigate(`/roadside-assistance/${garage.id}`);
  };
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden w-full max-w-full ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
      {/* Roadside Assistance Services Section */}
        ...
        <div className="max-w-7xl mx-auto relative z-10 w-full">
          <div className="text-center mb-8">
            ...
            <p className={`text-sm md:text-base ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
              Find 24/7 roadside assistance services near you
            </p>
          </div>
              ...
              {filteredGarages.length === 0 ? (
                <div className="text-center py-12">
                  <p className={`text-lg ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                    No roadside assistance services found matching your criteria.
                  </p>
                </div>
              ) : (
