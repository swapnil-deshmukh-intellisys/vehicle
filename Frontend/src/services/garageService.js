...
export const getGaragesByServiceCategory = async (requestData) => {
  ...
  try {
    ...
    // Map vehicleType to number of wheels
    const vehicleTypeWheeler = requestData.vehicleType === 'two-wheeler' ? 2 : requestData.vehicleType === 'four-wheeler' ? 4 : null;
    ...
    // Add vehicle_type_wheeler if available
    if (vehicleTypeWheeler) {
      apiRequestData.vehicle_type_wheeler = vehicleTypeWheeler;
    }
